...
void printHelp(){
    ...
    std::cout <<"In the case of nan-unified summary files all the files should be located in the same directory. Only the basename (full path without extension) is needed." << std::endl << std::endl; 
    ...
    std::cout << "-r \t\tChoosing regression test (not needed, default option)"<< std::endl;     
    std::cout << "-k keyword \tSpecify a specific keyword to compare, for example - k WOPR:PRODU1."<< std::endl;
    std::cout << "-p \t\tWill print the keywords of the files " << std::endl << std::endl; 
    ...
    std::cout << "-d \t\tThe program wull not throw an exception when the volume error ratio exceeds the limit" << std::endl; 
    std::cout << "-g \t\tWill print out the vector with the greatest error ratio" << std::endl; 
    std::cout << "-k keyword \tSpecify a specific keyword to compare, for example - k WOPR:PRODU1."<< std::endl; 
    ...
    std::cout << "-v \t\tFor the rate keywords WOPR, WGPR, WWPR and WBHP. Calculates the error volume of \n\t\tthe two summary files. Prints out to screen." << std::endl;
    std::cout << "-V keyword \tFor a specific keyword. Will calculate the error rate \n" << std::endl;
    std::cout << "Suggested combination of command line options"<< std::endl; 
    std::cout << " -i -g -m mainVariable, will print the vector which have the greatest error ratio of the mainVariable of interest\n"<< std::endl; 
}
